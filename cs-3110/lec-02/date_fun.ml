let validate_date d m =
  ...
  in
  d >= 1
  && match last_day with Some n -> d <= n | None -> m = "Feb" && d <= 28

let validate_date' d m =
  let last_day =
    if List.mem m ["Jan"; "Mar"; "May"; "Jul"; "Aug"; "Oct"; "Dec"] then Some 31
    else if List.mem m ["Apr"; "Jun"; "Sep"; "Nov"] then Some 30
    else None in
  d >= 1
  && match last_day with Some n -> d <= n | None -> m = "Feb" && d <= 28
...
let validate_date'' d m =
  ...
  let rec is_exceeded i = function
    | [] -> false
    | x :: xs ->
        if x <> m then is_exceeded (i + 1) xs
        else
          let i' = i mod 7 in
          if i' = 2 then d <= 28
          else if i' mod 2 = 1 then d <= 31
          else if i' mod 2 = 0 then d <= 30
          else false in
